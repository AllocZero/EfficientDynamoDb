...
{
    internal sealed class ByteEnumDdbConverter<TEnum> : DdbConverter<TEnum>, IDictionaryKeyConverter<TEnum>, ISetValueConverter<TEnum> where TEnum : struct, Enum
    {
        public ByteEnumDdbConverter() : base(true)
        {
        }
        ...
        {
            attributeValue = new AttributeValue(new NumberAttributeValue(Unsafe.As<TEnum, byte>(ref value).ToString()));
            return true;
        }

        public override AttributeValue Write(ref TEnum value) => new AttributeValue(new NumberAttributeValue(Unsafe.As<TEnum, byte>(ref value).ToString()));
        ...
        public void WritePropertyName(in DdbWriter writer, ref TEnum value)=> writer.JsonWriter.WritePropertyName(Unsafe.As<TEnum, byte>(ref value));

        public void WriteStringValue(in DdbWriter writer, ref TEnum value) => writer.JsonWriter.WriteStringValue(Unsafe.As<TEnum, byte>(ref value));
